...
class ScoreDetections(caffe.Layer):
    ...
    * top[0]- Marked up bbox
    ...
    def reshape(self, bottom, top):
        n_images = bottom[0].data.shape[1]
        num_classes = bottom[0].data.shape[0]
        # Assuming that max booxes per image are MAX_BOXES
        top[0].reshape(num_classes, n_images, MAX_BOXES, 5)
        assert(bottom[0].data.shape[1] == bottom[1].data.shape[1]), "# of images not matching!"
...
def score_det(gt_bbox_list, det_bbox_list):
    threshold = 0.7
    matched_bbox = np.zeros([gt_bbox_list.shape[0], gt_bbox_list.shape[1], MAX_BOXES, 5])

    for kk in range(gt_bbox_list.shape[0]):
        for k in range(gt_bbox_list.shape[1]):

            # Remove  zeros from detected bboxes
            cur_det_bbox = det_bbox_list[kk, k, :, 0:4]
            cur_det_bbox = np.asarray(filter(lambda a: a.tolist() != [0, 0, 0, 0], cur_det_bbox))

            # Remove  zeros from label bboxes
            cur_gt_bbox = gt_bbox_list[kk, k, :, 0:4]
            ...
            matched_bbox[kk, k, 0:temp.shape[0], :] = temp
